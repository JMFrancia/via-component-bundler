...
function activate(inputs) {
    ...
    makeDirectory(inputs.targetDir, inputs.packageName).then(function() {
        makeDirectory(mainDir, 'src').then(function() {
            ...
            //Do this once /src & /test directories created
            copyToNewFile('./baseFiles/componentIndex.js', srcDir, 'index.ts').catch(function(err) {
                handleError(err);
            }).then(function() {
                ...
                getFileNames(inputs.componentDir).forEach(function(file) {
                    ...
                    copyToNewFile(fileLoc, dest, file).catch(function(err) {
                        handleError(err);
                    });
        ...
        copyToNewFile('./baseFiles/README.md', mainDir, 'README.md').catch(function(err) {
            handleError(err);
        }).then(
            function(targetFile) {
                fillOutBaseFile(targetFile, templateDefs.readme).catch(function(err) {
                    handleError(err);
                });
            }
        );
        ...
        copyToNewFile('./baseFiles/package.json', mainDir, 'package.json').catch(function(err) {
            handleError(err);
        }).then(function(targetFile) {
            fillOutBaseFile(targetFile, templateDefs.package).catch(function(err) {
                handleError(err);
            });
        });
    }).catch(function(err) {
